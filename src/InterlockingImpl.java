 ...
 * Tuned freight logic to handle passenger priority correctly.
 */
public class InterlockingImpl implements Interlocking {
    ...
    public int moveTrains(String... names) {
        ...
        for (String n : ordered) {
            ...
            // --- Freight coordination (3,4,7,11) final tuned ---
            if ((cur == 3 && next == 4) || (cur == 4 && next == 3) ||
                (cur == 7 && next == 3) || (cur == 3 && next == 7)) {

                // check if any passenger train is moving or waiting to move through 5/6/10
                boolean passengerMoving = false;
                for (String tn : moveSet) {
                    if (isPassengerTrain(tn)) {
                        int pos = trainLocations.getOrDefault(tn, -1);
                        int nxt = getNextSectionForTrain(tn);
                        if (pos == 5 || pos == 6 || pos == 10 || nxt == 5 || nxt == 6 || nxt == 10) {
                ...
                // if passenger trains are active in mainline corridor, wait
                if (passengerMoving) continue;

                // otherwise freight can safely move now if no head-on conflict
